...
class MarkovChain(object):
    ...
    def __init__(self,
        ...
        :param single_state_prediction: tansition_matrix shaped n_possible_states_order by number_of_states
        """
        self.number_of_states = n_states
        self.order = order
        self.verbose = verbose
        self.single_state_prediction = single_state_prediction

        # creates a map of state and label
        # (necessary to recover state label in High Order MC)
        if not self.single_state_prediction:
            ...
            # (necessary to recover state label in High Order MC)
            self.possible_states_row = {
                j: i for i, j in
                enumerate(itertools.product(range(self.number_of_states), repeat=order))
            }
            self.possible_states_col = {
                j: i for i, j in
                enumerate(itertools.product(range(self.number_of_states), repeat=1))
            }
            # print(f"possible_states:\t{possible_states_row}")
            # allocate transition matrix
            self.transition_matrix = sparse.dok_matrix((
                (len(self.possible_states_row), len(self.possible_states_col))
            ), dtype=np.float64)
    ...
    def _update_transition_matrix(self, states_sequence, normalize=True):
        ...
        # print(f"Visted states: {visited_states}")
        for state_index, i in enumerate(visited_states):
            try:
                if self.single_state_prediction:
                    self.transition_matrix[
                        self.possible_states_row[tuple(i)],
                        self.possible_states_col[(visited_states[
    ...
    def transition_df(self):
        ...
        """
        # print(f"transition_matrix: {type(self.transition_matrix)}")
        sdf = pd.DataFrame.sparse.from_spmatrix(self.transition_matrix)

        if not self.single_state_prediction:
            sdf.index = sorted(self.possible_states)
            sdf.columns = sorted(self.possible_states)
        else:
            sdf.index = sorted(self.possible_states_row)
            sdf.columns = sorted(self.possible_states_col)
    ...
    def possible_states_lookup(self):
        ...
        if not self.single_state_prediction:
            ...
            return (
                {v: k for k, v in self.possible_states_row.items()},
                {v: k for k, v in self.possible_states_col.items()}
            )
